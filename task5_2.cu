...
int main(int argc, char* argv[]) {
    ...
    while (err > accuracy && iter < iterations) 
    {
      ...
      if (iter % 100 == 0) {
        ...
        ncclAllReduce((void*)max_err,(void*)max_err, 1, ncclDouble, ncclMax, ncclcomm, stream);
      ...
      if (rank != 0)
      {
          ncclSend(CudaArr + Matrix + 1, Matrix - 2, ncclDouble, rank - 1, ncclcomm, stream); 
          ncclRecv(CudaArr + 1, Matrix - 2, ncclDouble, rank - 1, ncclcomm, stream);

      }
          // Обмен нижней границей
      if (rank != size - 1)
      {
          ncclSend(CudaArr + (size_y - 2) * Matrix + 1, Matrix - 2, ncclDouble, rank + 1, ncclcomm, stream);
          ncclRecv(CudaArr + (size_y - 1) * Matrix + 1, Matrix - 2, ncclDouble, rank + 1, ncclcomm, stream);
      }
          ncclGroupEnd();           
